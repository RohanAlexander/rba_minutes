...
library(tidyverse)
address <- read_csv("inputs/addresses.csv")
...
data_to_scrape <- read_csv("inputs/addresses.csv") # Just a list of the URLs and dates for each minutes.
...
visit_address_and_save_content <-
  function(name_of_address_to_visit,
    ...
    Sys.sleep(sample(30:60, 1)) # Space out each request by somewhere between 30 and 60 seconds each so that we don't overwhelm their server
  }
...
# You can also try setting K to zero within STM and seeing the number of topics that it recommends: e,g, choose_topic_num_for_me <- stm(rba_minutes, K = 0, verbose = FALSE)
